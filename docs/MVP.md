...
- **Faza 2 — Gmail:** własny cienki MCP Gmail (per-user token injection) — ✅ v1 readonly (`gmail_search_messages` / `gmail_list_messages` / `gmail_get_message`). UI: wyniki Gmail mapowane na rich blocks (`card` dla `gmail_get_message`, `table` dla search/list) — ten sam pipeline co GitHub. Teams odłożony (brak dostępu). GitHub już gotowy z Fazy 1.
...
- **Faza 4 — Pamięć + RAG:** magazyn wektorowy, memory injection (semantic + tool), RAG. **Fundament ✅** — pamięć (embeddingi + `memory_search`/`memory_save`/`memory_update`) + document RAG (`rag_documents`/`document_chunks`, `rag_search`, ACL tenant/user); patrz plany [005](plans/2026-07-23--005--memory-update.md), [006](plans/2026-07-23--006--rag-retrieval-chunks.md). **Jakość — plan [009](plans/2026-07-25--009--phase-4-retrieval-quality.md): infrastruktura ✅, `verification needed`** — embed-versioning (migracja 065), `embed_batch`+retry+cache, hybrid search (`tsvector`+RRF, ACL zweryfikowane na realnym Postgresie), `Reranker` Protocol (Noop domyślnie), `split_markdown()`, async ingest (202/pending→ready/failed), attachment ingest, memory dedupe, UI Knowledge (`/workspace/knowledge`, zweryfikowane w przeglądarce), `backend/evals/rag/` (34 pytania PL/EN, 4 metryki retrievalu). **Brakuje jednego kroku:** brak żywego `OPENROUTER_API_KEY` w środowisku implementacji — finalny wybór modelu embeddingów (`cohere/embed-v4` vs `google/gemini-embedding-001`, patrz Rozstrzygnięcia niżej) wymaga uruchomienia eval-harness z realnym kluczem. Podstawa decyzyjna: [research 008](research/2026-07-25--008--embeddings-rag-second-brain.md).
- **Faza 4.5 — Second Brain:** wiki/wiedza — `wiki_pages`/`wiki_links`, toole bibliotekarza (`wiki_ingest`/`wiki_query`/`wiki_lint`), przeglądarka folderów + graf. **pgvector-only** — strony wiki idą tym samym pipeline'em chunków co reszta RAG; **LightRAG: no-go** (research 008 §7). Plan [010](plans/2026-07-25--010--phase-4-5-second-brain-wiki.md) — **✅ done (2026-07-27)**. Osobno, bramkowany: memory graph / Graphiti — plan [011](plans/2026-07-25--011--memory-graph-graphiti.md): etap 1 facade **✅**, etap 2 spike scaffolding **✅** (FalkorDB :6383, `GraphitiMemoryBackend`, runner), bramka metryk czeka na żywy `OPENROUTER_API_KEY` — research [009](research/2026-07-27--009--graphiti-spike-gate.md); etapy 3+ tylko po go i zgodzie.
- **Faza 5 — Rozszerzenia:** auto-router LLM, tool search, onboarding tenantów (multi-tenancy B), więcej bloków. **Web search ✅** (poza kolejnością, plan [013](plans/2026-07-28--013--web-search.md)): `web_search`/`web_fetch` w trybie hybrydowym (`server` = server tools OpenRoutera, zero-config / `local` = własne toole + Tavily), blok `sources` domykający „odnośniki do źródeł" z dec. #14, guard SSRF, kaskada `web_search_enabled`. `verification needed` — bramka `ws-probe` czeka na żywy `OPENROUTER_API_KEY`.
...
- **Web search** (2026-07-28) — agent dostaje dostęp do internetu; plan [013](plans/2026-07-28--013--web-search.md).
  - **Architektura: hybryda za jednym przełącznikiem** `AI_WEB_SEARCH_MODE`. `server` (domyślny) dokleja server toole OpenRoutera (`openrouter:web_search`/`openrouter:web_fetch`) do tablicy `tools` — zero nowych kluczy, ale wyszukiwanie dzieje się poza naszą pętlą, więc nie ma kroków na żywo w SSE (jest syntetyczny krok po turze). `local` rejestruje własne `WebSearchTool`/`WebFetchTool` — pełny trace, cache w Redisie, miejsce na quoty. Oba tryby zbiegają się na tym samym bloku `sources`.
  - **Provider `local`: Tavily** — `/search` + `/extract` z jednego klucza; `/extract` zwraca gotową treść, więc backend **nie dostaje zależności do parsowania HTML**. Za `WebSearchProvider` (Protocol) — Brave/Exa wchodzą bez zmian w warstwie narzędzi.
  - **Cytowania:** nowy typ bloku `sources`, konwencja jak przy `chart` — dowolny tool zwracający `{"sources": [...]}` dostaje blok. **Domyka dec. #14** („odnośniki do źródeł"), która była zadeklarowana i nigdy nie zbudowana.
  - **Ingest:** przez istniejący `wiki_ingest` (przyjmował `source_url`, ale nikt go nie pobierał) — łańcuch `web_search → web_fetch → wiki_ingest` bez nowego storage'u.
  - **Domyślnie włączone** (`WORKSPACE_DEFAULT_WEB_SEARCH_ENABLED=true`) — odstępstwo od `rag_enabled=false`, bo tryb `server` nie wymaga konfiguracji. Świadomy koszt: wyniki są rozliczane per wynik (~$4/1000). Kaskada pozwala wyłączyć na dowolnym poziomie.
...
- **Limity kosztów narzędzi zewnętrznych** (2026-07-28, z planu 013) — **częściowo zamknięte** (plan [014](plans/2026-07-29--014--openrouter-usage-metering.md)): ledger `usage_events`, cap USD platformy + sub-cap web search, kaskada `monthly_cost_cap_usd` / `monthly_web_search_cap`. Pełne Stripe tenant → plan [015](plans/2026-07-29--015--stripe-pro-workspace.md).
