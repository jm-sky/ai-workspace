...
- **Limity kosztów narzędzi zewnętrznych** (2026-07-28, z planu 013) — `web_search` jest pierwszą zdolnością rozliczaną **per użycie**, a nie per token. Kaskada `web_search_enabled` daje wyłącznik zero-jedynkowy, ale nie ma twardego capa (dziennego/miesięcznego, per tenant). `modules/feature_limits/` istnieje z czasów gear-stacku i nie jest podpięty do warstwy narzędzi; kolumny `limits`/`guardrails` w tabeli agentów są puste. Do rozstrzygnięcia: gdzie liczyć zużycie (trace runu vs osobny licznik) i co robić po przekroczeniu (miękki komunikat vs twarde wyłączenie toola).
